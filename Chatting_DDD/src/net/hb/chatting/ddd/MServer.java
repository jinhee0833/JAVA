...
public class MServer extends JFrame implements Runnable, ActionListener{  //스레드 사용
	...
	public MServer() {
		super("서버");
		System.out.println("===Mserver 생성자 12:30");
		this.add(ta,"Center");
	...
	public void run(){
		...
		System.out.println("===run() 생성자 12:30");
		try{
		...
		while(true){
			try{
				...
				cs.messageAll(cs.myname); //클라이언트의 이름을 모든 클라이언트에게 뿌려 입장했음을 알린다. 왜냐면 /c대화명이 전달이 되니까 일단 입장만 알리는거죠.
				vc.add(cs); //이미 접속되어 있는 다른 클라이언트에게 나를 알리고 나를 벡터에 등록

				for(int i=0;i<vc.size();i++){
					Service cs1=(Service)vc.elementAt(i); //벡터 요소 꺼내기
					cs.message(cs1.myname); //myname을 가지고 클라이언트에 메시지 뿌려주기
	...
	class Service extends Thread{  //내부클래스...Run()메소드 재정의. 이름이나 /n과 같은 명령어, 메시지를 받고 뿌리는 역할인듯...
		...
		public void messageAll(String msg){
			System.out.println("===messageALL 생성자 12:30");
		...
		public void message(String msg) throws Exception{
			System.out.println("===message 생성자 12:30");
	...
	public void actionPerformed(ActionEvent e) {
		System.out.println("===actionPerformed 생성자 12:30");
